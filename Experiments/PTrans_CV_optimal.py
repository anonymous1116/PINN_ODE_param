...
def main(args):
    ...
    output_dir = f"../depot_hyun/hyun/ODE_param/PTrans_sig{sci_str}/CV_optimal"
    ...
    penalty_list = [0.00001, 0.00005, 0.0001, 0.0005, 0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1, 5] #sigma = 0.5
    
    CV_error_list = []
    for j in range(len(penalty_list)):
        penalty = penalty_list[j]
        penalty_char = format(penalty, ".1e").replace(".", "_")
        CV_dir = f"../depot_hyun/hyun/ODE_param/PTrans_sig{sci_str}/lambda_{penalty_char}_CV"
    ...
    for epoch in range(train_epochs):
        ...
        for i in range(0, n, variable_batch_size):
            ...
            batch_loss = model.compute_loss(
                derivative_batch_t=[s.reshape(-1, 1) for s in train_generator.get_examples()],  # list([100, 1])
                variable_batch_t=[t[variable_batch_id].view(-1, 1)],  # list([10, 1])
                batch_y=y_interpolate[variable_batch_id],  # [10, 5]
